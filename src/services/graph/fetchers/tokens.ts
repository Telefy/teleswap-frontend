...
import { Currency, CurrencyAmount } from '@telefy/teleswap-core-sdk'
import { SupportedChainId as ChainId } from 'constants/chains'
import { STABLECOIN_AMOUNT_OUT } from 'hooks/useUSDCPrice'
import { fetcher } from 'services/graph'
import { getTridentTokenPriceQuery, getTridentTokenPricesQuery, getTridentTokensQuery } from 'services/graph/queries'
...
const formatCurrencyAmounts = (chainId: ChainId, tokenPrices: TokenPrice[]) => {
  return tokenPrices.map(({ derivedUSD }) => {
    ...
    return CurrencyAmount.fromRawAmount(STABLECOIN_AMOUNT_OUT[chainId].currency, Number(derivedUSD).toFixed(decimals))
  })
}

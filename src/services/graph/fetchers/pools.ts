...
import { Fee, PoolType } from '@telefy/teleswap-trident-sdk'
...
export const fetcher = async <T>(chainId = ChainId.MAINNET, query, variables: {} = undefined): Promise<T> => {
...
// @ts-ignore TYPE NEEDS FIXING
export const getTridentPoolTransactions = async (chainId: ChainId = ChainId.MAINNET, variables) => {
...
// @ts-ignore TYPE NEEDS FIXING
export const getPoolKpis = async (chainId: ChainId = ChainId.MAINNET, variables = {}): Promise<PoolKpi[]> => {
...
export const getPoolKpi = async (chainId: ChainId = ChainId.MAINNET, variables = {}): Promise<PoolKpi> => {
  const result: PoolKpiQueryResult = Object.values(

...
export const getAprData = (pool: DeserializedPool, performanceFee: number) => {
  ...
  if (vaultKey) {
    const autoApr = getApy(apr || 0, autoCompoundFrequency, 365, performanceFee) * 100
...
export const getPoolBlockInfo = (pool: DeserializedPool, currentBlock: number) => {
  ...
  const blocksUntilStart = Math.max((startBlock || 0) - currentBlock, 0)
  const blocksRemaining = Math.max((endBlock || 0) - currentBlock, 0)

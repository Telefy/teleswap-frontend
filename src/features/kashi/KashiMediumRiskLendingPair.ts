...
import { accrue, AccrueInfo, computePairAddress, interestAccrue, takeFee } from '@telefy/teleswap-tele-sdk'
...
export class KashiMediumRiskLendingPair {
  ...
  public get maxAssetAvailable(): JSBI {
    return minimum(
      JSBI.greaterThan(this.currentAllAssetShares, JSBI.BigInt(0))
  ...
  public get maxAssetAvailableFraction(): JSBI {
    if (JSBI.equal(this.currentAllAssets, JSBI.BigInt(0))) return JSBI.BigInt(0)
  ...
  public get marketHealth(): JSBI {
    if (JSBI.equal(this.currentBorrowAmount, JSBI.BigInt(0)) || JSBI.equal(this.maximumExchangeRate, JSBI.BigInt(0))) {
  ...
  public get currentUserAssetAmount(): JSBI {
    if (JSBI.equal(this.totalAsset.base, JSBI.BigInt(0))) return JSBI.BigInt(0)
  ...
  public get currentUserBorrowAmount(): JSBI {
    if (JSBI.equal(this.totalBorrow.base, JSBI.BigInt(0))) return JSBI.BigInt(0)
  ...
  public get maxBorrowable() {
    const max = {
      oracle: JSBI.greaterThan(this.oracleExchangeRate, JSBI.BigInt(0))
        ? JSBI.divide(
          JSBI.multiply(this.userCollateralAmount, JSBI.multiply(JSBI.BigInt(1e16), JSBI.BigInt(75))),
          this.oracleExchangeRate
        )
        : JSBI.BigInt(0),
      spot: JSBI.greaterThan(this.spotExchangeRate, JSBI.BigInt(0))
        ? JSBI.divide(
          JSBI.multiply(this.userCollateralAmount, JSBI.multiply(JSBI.BigInt(1e16), JSBI.BigInt(75))),
          this.spotExchangeRate
        )
        : JSBI.BigInt(0),
      stored: JSBI.greaterThan(this.exchangeRate, JSBI.BigInt(0))
        ? JSBI.divide(
          JSBI.multiply(this.userCollateralAmount, JSBI.multiply(JSBI.BigInt(1e16), JSBI.BigInt(75))),
          this.exchangeRate
        )
  ...
  public get health(): JSBI {
    if (JSBI.equal(this.maxBorrowable.minimum, JSBI.BigInt(0))) return JSBI.BigInt(0)
  ...
  public simulatedHealth(borrowAmount: JSBI = JSBI.BigInt(0), collateralAmount: JSBI = JSBI.BigInt(0)): JSBI {
    ...
    if (JSBI.equal(minimum, JSBI.BigInt(0))) return JSBI.BigInt(0)

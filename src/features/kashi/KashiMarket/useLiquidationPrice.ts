...
import { Currency, CurrencyAmount, Price, TradeType } from '@telefy/teleswap-core-sdk'
import { Trade as LegacyTrade } from '@mazelon/teleswap-sdk'
import JSBI from 'jsbi'
import { LTV } from 'features/kashi/constants'
...
export const useLiquidationPrice: UseLiquidationPrice = ({ borrowAmount, collateralAmount, invert, trade, reduce }) => {
  ...
  try {
    ...
    return invert
      ? `1 ${totalBorrowed?.currency.symbol} = ${liquidationPrice?.toSignificant(6)} ${totalCollateral?.currency.symbol
      }`
      : `1 ${totalCollateral?.currency.symbol} = ${liquidationPrice?.invert().toSignificant(6)} ${totalBorrowed?.currency.symbol
      }`
  } catch (e) {
    console.log(e)
    return 'Instant liquidation'

...
export const getCakeVaultEarnings = (
  ...
  const hasAutoEarnings =
    account && teleAtLastUserAction && teleAtLastUserAction.gt(0) && userShares && userShares.gt(0)
  const { cakeAsBigNumber } = convertSharesToCake(userShares, pricePerFullShare)
  const autoCakeProfit = cakeAsBigNumber.minus(fee || BIG_ZERO).minus(teleAtLastUserAction)
...
export const getPoolBlockInfo = (pool: DeserializedPool, currentBlock: number) => {
  ...
  const blocksUntilStart = startBlock ? Math.max(startBlock - currentBlock, 0) : 0
  const blocksRemaining = endBlock ? Math.max(endBlock - currentBlock, 0) : 0

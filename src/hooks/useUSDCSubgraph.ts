...
import { Currency, CurrencyAmount, Price, Token } from '@telefy/teleswap-core-sdk'
import { USDC } from 'constants/tokens'
import { useNativePrice, useTokens, useTridentTokens } from 'services/graph'
...
export function useUSDCPricesSubgraph(
  ...
  const stablecoin = chainId ? CurrencyAmount.fromRawAmount(USDC, 0).currency : undefined
  ...
  return useMemo(() => {
    ...
    currencies.map((currency) => {
      ...
      const tokenTrident = tokensTrident?.find((token: any) => token.id === currency.wrapped.address.toLowerCase())
